...
export default class AuthService {
    ...
    return MUTEX.runExclusive(() => {
      ...
      if (res.status === 401) {
        ...
        if (latestAuthToken != authToken) {
          return MUTEX.runExclusive(() => {
            return this._doAuthFetch(url, options, latestAuthToken);
          });
        } else {
          this._removeAuthToken();
          this._removeAuthTokenExpirationTime();
      ...
      .then((object) => {
        if (object.status === 200) {
          ...
          let tokenExpirationTime =
            this._getAuthTokenExpirationTimeFromResponse(object.body);
    ...
    return this._authFetch(this.LOGIN_ENDPOINT, {
      ...
      .then((res) => {
        ...
        if (token) {
          ...
          let tokenExpirationTime =
            this._getAuthTokenExpirationTimeFromResponse(res);
          this._setAuthTokenExpirationTime(tokenExpirationTime);
          return { result: true };
        } else {
          this._removeAuthToken();
  ...
  needsRegistration = () => {
    return fetch(this.REGISTRATION_STATUS_API_ENDPOINT, { method: "GET" })
      .then((response) => response.json())
      .then((res) => {
  ...
  _setAuthTokenExpirationTime(tokenExpirationTime) {
    ...
    localStorage.setItem(
      this.TOKEN_TTL_NAME_IN_LOCALSTORAGE,
      currentDateTimeSeconds + tokenExpirationTime * 0.7,
    );
  }
