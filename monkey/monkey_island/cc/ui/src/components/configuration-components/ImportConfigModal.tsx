...
import UnsafeConfigOptionsConfirmationModal from "./UnsafeConfigOptionsConfirmationModal.js";
...
const BAD_CONFIGURATION_MESSAGE =
  "Configuration file is corrupt or in an outdated format.";
...
const ConfigImportModal = (props: Props) => {
  ...
  const [showUnsafeOptionsConfirmation, setShowUnsafeOptionsConfirmation] =
    useState(false);
  ...
  function decryptAndSetConfig() {
    try {
      ...
      let decryptedConfigCredentials = JSON.parse(
        decryptText(configCredentials, password),
      );
      let decryptedConfigMasqueStrings = JSON.parse(
        decryptText(configMasqueStrings, password),
      );
      ...
      setUploadStatus(UploadStatuses.error);
      setErrorMessage(
        "Decryption failed: Password is wrong or the file is corrupted",
      );
    }
  }
  ...
  function submitImport() {
    ...
    Promise.all([
      configurationSubmissionStatus,
      credentialsSubmissionStatus,
      masqueStringsSubmissionStatus,
  ...
  const submitConfigMasqueStringsToServer = () => {
    const { linuxMasqueBytes, windowsMasqueBytes } =
      getMasqueradesBytesArrays(configMasqueStrings);

    let linuxMasqueStringsSubmissionStatus = sendConfigMasqueStringsToServer(
      APIEndpoint.linuxMasque,
      linuxMasqueBytes,
    );
    let windowsMasqueStringsSubmissionStatus = sendConfigMasqueStringsToServer(
      APIEndpoint.windowsMasque,
      windowsMasqueBytes,
    );

    return (
      linuxMasqueStringsSubmissionStatus && windowsMasqueStringsSubmissionStatus
    );
  };
  ...
  function sendConfigMasqueStringsToServer(endpoint, masqueBytes) {
    return IslandHttpClient.put(endpoint, masqueBytes, true).then((res) => {
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (e instanceof TypeError) {
          setErrorMessage(
            "Missing required fields; configuration file is most " +
...
const PasswordInput = (props: { onChange: (passValue) => void }) => {
  return (
