...
export default function applyUiSchemaManipulators(
  selectedSection,
  formData,
  uiSchema,
  JSONSchema,
...
function ransomwareDirManipulator(selectedSection, formData, uiSchema) {
...
function pluginsDirManipulator(
  selectedSection,
  formData,
  uiSchema,
  JSONSchema,
  ...
  if (selectedSection === PROPAGATION_TAB) {
    pluginsManipulator(
      uiSchema?.exploitation?.exploiters,
      JSONSchema?.properties?.exploitation?.properties?.exploiters?.properties,
    );
  } else if (selectedSection === CREDENTIAL_COLLECTORS_TAB) {
    pluginsManipulator(
      uiSchema?.credentials_collectors,
      JSONSchema?.properties?.credential_collectors?.properties,
    );
  }
}
