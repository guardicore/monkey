...
const PLUGIN_SCHEMA_PATH = {
    propagation: EXPLOITERS_PATH_PROPAGATION,
...
export default function PluginSelectorTemplate(props: ObjectFieldTemplateProps) {
    ...
    const updateUISchema = () => {
        ...
        for (let pluginName of Object.keys(defaultSchema)) {
            uiSchema[pluginName] = Object.assign(
                { ...uiSchema[pluginName] },
                { 'ui:readonly': !props.formContext.selectedPlugins.has(pluginName) }
            );
        }
    ...
    function isPluginSafe(itemKey) {
        let itemSchema = Object.entries(props.schema.properties).filter(
            (e) => e[0] == itemKey
        )[0][1];
    ...
    function onResetClick() {
        let safePluginNames = [...props.formContext.selectedPlugins].filter((pluginName) =>
            isPluginSafe(pluginName)
        );
    ...
    return (
        <div className={'advanced-multi-select'}>
            <AdvancedMultiSelectHeader
                ...
                checkboxState={getMasterCheckboxState([...props.formContext.selectedPlugins])}
                hideReset={getHideResetState([...props.formContext.selectedPlugins])}
