...
import { IdentityType, SECRET_TYPES } from "../utils/CredentialTitle.tsx";
...
export function reformatConfig(config, reverse = false) {
  ...
  if (reverse) {
    formattedConfig["keep_tunnel_open_time"] =
      formattedConfig["advanced"]["keep_tunnel_open_time"];
    delete formattedConfig["advanced"];

    formattedConfig["propagation"]["maximum_depth"] =
      formattedConfig["propagation"]["general"]["maximum_depth"];
...
export function reformatSchema(schema) {
  ...
  schema["properties"]["propagation"]["properties"]["general"] = {
    ...
    properties: {
      maximum_depth:
        schema["properties"]["propagation"]["properties"]["maximum_depth"],
    },
  };
...
export function formatCredentialsForForm(credentialsData) {
  ...
  for (const item of credentialsData) {
    ...
    const identityKey =
      identity?.[IdentityType.Username] ||
...
export function formatCredentialsForIsland(credentialsData) {
  ...
  credentialsData.forEach((row) => {
    ...
    const identityType = identityValue
      ? isEmail(row.identity)
    ...
    if (
      identityObj &&
      (secretValue[SECRET_TYPES.ssh_private_key] ||
        secretValue[SECRET_TYPES.ssh_public_key])
    ) {
      formattedCredentials.push(getCredentialPair(identityObj, secretValue));
    }

    CREDENTIALS_ROW_KEYS.filter((key) => !keysToIgnore.includes(key)).forEach(
      (key) => {
        const currentSecretObj =
          row[key] !== "" ? { [SECRET_TYPES[key]]: row[key] } : null;
        if (currentSecretObj || identityObj) {
          if (
            !currentSecretObj &&
            identitiesWithEmptySecret.includes(identityValue)
          ) {
            return;
          }
          formattedCredentials.push(
            getCredentialPair(identityObj, currentSecretObj),
          );
        }
        if (
          identityObj &&
