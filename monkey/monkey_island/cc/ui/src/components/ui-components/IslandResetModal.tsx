...
const IslandResetModal = (props: Props) => {
    ...
    function clearSimulationData(callback: () => void) {
        auth.authFetch('/api/clear-simulation-data', { method: 'POST' }).then((res) => {
            if (res.ok) {
                callback();
            }
        });
    ...
    function resetAll(): Promise<void> {
        return auth
            .authFetch('/api/reset-agent-configuration', { method: 'POST' })
            .then((res) => {
                if (res.ok) {
                    return auth.authFetch('/api/clear-simulation-data', {
                        method: 'POST'
                    });
                }
            })
            .then((res) => {
                if (res.ok) {
                    return auth.authFetch('/api/propagation-credentials/configured-credentials', {
                        method: 'PUT',
                        body: '[]'
                    });
                }
            ...
            .then((res) => {
                if (res.ok) {
                    return auth
                        .authFetch('/api/agent-binaries/linux/masque', { method: 'PUT' })
                        .then((res) => {
                            if (res.ok) {
                                return auth.authFetch('/api/agent-binaries/windows/masque', {
                                    method: 'PUT'
                                });
                            }
                        });
    ...
    function showModalButtons() {
        return (
            <Container className={`text-left island-reset-modal`}>
                <Row>
                    <Col>
                ...
                <Row>
                    <Col>
                        ...
                        <p>
                            You might want to{' '}
                            <Button variant={'link'} href={IslandRoutes.ConfigurePage}>
