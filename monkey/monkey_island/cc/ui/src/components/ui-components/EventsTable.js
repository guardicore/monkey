    ...
    if (_.has(tempObj, stolenCredentialsFieldName)) {
        for (let stolenCredential of tempObj[stolenCredentialsFieldName]) {
            ...
            for (let secretType in secrets) {
                let redactedSecret = '*'.repeat(
                    stolenCredential[secretFieldName][secretType].length
                );
...
const EventsTable = () => {
    ...
    useEffect(() => {
        ...
        IslandHttpClient.getJSON(APIEndpoint.agents, {}, true).then((res) => setAgents(res.body));

        IslandHttpClient.getJSON(APIEndpoint.machines, {}, true).then((res) =>
            setMachines(res.body)
        );

        IslandHttpClient.getJSON(APIEndpoint.agentEvents, {}, true).then((res) => {
            setLoading(false);
            setEvents(res.body);
        });
    }, []);
