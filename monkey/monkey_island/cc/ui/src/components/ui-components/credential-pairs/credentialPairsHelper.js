...
export const CREDENTIALS_ROW_KEYS = [
  "identity",
  "password",
  "lm",
  "ntlm",
  ...
  return [
    ...
    },
    {
      ...
      preProcessEditCellProps: (params) => {
        ...
        if (setErrorForRow) {
          hasError
            ? setErrorForRow(params.id, true)
            : setErrorForRow(params.id, false);
        }
...
export const updateFilterAndSortPropertiesToColumn = (
  col,
  filterable = true,
  sortable = true,
) => {
...
export const getDataColumns = (
  getRowActions,
  disableAllColumnsFilterAndSort,
  setErrorForRow,
  ...
  return mainColumns.map((col) => {
    if (disableAllColumnsFilterAndSort) {
      ...
      return col.field === "identity"
        ? updateFilterAndSortPropertiesToColumn(col)
        : updateFilterAndSortPropertiesToColumn(col, false, false);
    }
  });
};
...
export const isAllValuesInRowAreEmpty = (
  row,
  keysToIgnore = [],
  isRowAnEditMode = false,
) => {
  if (row) {
    for (const key of CREDENTIALS_ROW_KEYS) {
