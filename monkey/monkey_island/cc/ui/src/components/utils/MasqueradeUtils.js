  ...
  if (stringsArray?.length) {
    ...
    if (masquePrefix === MASQUE_TYPES.TEXTS.prefix) {
      bytes = stringsArray
        ...
        .reduce(
          (accumulator, currentString) => [...accumulator, ...currentString],
          [],
        );
    } else {
...
export function getStringsFromBytes(bytesArray, masqueType, masqueDetails) {
  if (bytesArray && bytesArray?.byteLength && masqueDetails) {
    ...
    var uint8Array = splitUint8ArrayByNull(
      new Uint8Array(dataViewArray.buffer),
    ).slice(-1)[0];
    return (
      bytesToBase64(uint8Array) || getDefaultValueByMasquePrefix(masqueType.key)
    );
  }
...
const getDefaultValueByMasquePrefix = (masqueType) => {
  return Object.entries(cloneDeep(DEFAULT_MASQUES_VALUES)).filter(
    ([key, _defaultValue]) => {
...
export const getMasqueradesBytesArrays = (masqueStrings) => {
  ...
  OS_TYPES.forEach((osType) => {
    ...
    Object.values(MASQUE_TYPES).forEach(({ key, prefix }) => {
      ...
      bytesArray = [
        ...bytesArray,
        ...transformStringsToBytes(currentVal, prefix),
      ];
    });
...
export const getMasqueradeBytesSubsets = (bytesArray) => {
  ...
  subsetsDetails = calculateMasquesSubsetsLengths(
    subsetsDetails,
    bytesArray.byteLength,
  );
...
const calculateMasquesSubsetsLengths = (subsetsDetails, bytesArrayLength) => {
  ...
  const keys = Object.keys(subsetsDetailsToReturn).sort(
    (a, b) =>
      subsetsDetailsToReturn[a].offsetIndex -
      subsetsDetailsToReturn[b].offsetIndex,
  );
  for (let i = 0; i < keys.length; i++) {
    ...
    if (i < keys.length - 1) {
      obj.length = Math.abs(
        subsetOffset - subsetsDetailsToReturn[keys[i + 1]].offsetIndex,
      );
    } else {
