...
export function getAllTunnels(agents, machines) {
  ...
  for (let machine of machines) {
    if (machine.island === true) {
      islandIPs = islandIPs.concat(...getMachineIPs(machine));
    }
  }
...
export function tunnelIssueReportByMachine(machineId, allTunnels) {
  if (allTunnels.length > 0) {
    ...
    if (tunnelIssuesByMachine.length > 0) {
      return (
        <>
          Use micro-segmentation policies to disable communication other than
          the required.
          <CollapsibleWellComponent>
            Machines are not locked down at port level. Network tunnels were set
            up between the following.
            <ul>{tunnelIssuesByMachine}</ul>
          </CollapsibleWellComponent>
        </>
      );
    }
  ...
  return (
    <>
      <span className="badge text-bg-primary">
        {tunnelMachineInfo.hostname}
      </span>{" "}
      (
      <span className="badge text-bg-info" style={{ margin: "2px" }}>
        {tunnelMachineInfo.ip}
      </span>
      )
    </>
  );
...
function getTunnelIssuesByMachine(machineId, allTunnels) {
  ...
  for (let tunnel of allTunnels) {
    if (
      tunnel.agent_machine.id === machineId ||
      tunnel.tunnel_machine.id === machineId
    ) {
      let agentMachineNameComponent = machineNameComponent(
        tunnel.agent_machine,
      );
      let tunnelMachineNameComponent = machineNameComponent(
        tunnel.tunnel_machine,
      );
      uniqueTunnels.push({
  ...
  for (let tunnel of uniqueTunnels) {
    tunnelIssues.push(
      <li
        key={
          tunnel.agentMachineNameComponent + tunnel.tunnelMachineNameComponent
        }
      >
        from {tunnel.agentMachineNameComponent} to{" "}
        {tunnel.tunnelMachineNameComponent}
      </li>,
    );
  }
