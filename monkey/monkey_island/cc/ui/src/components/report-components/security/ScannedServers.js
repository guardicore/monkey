...
function ScannedServersComponent(props) {
    ...
    useEffect(() => {
        getCollectionObject(APIEndpoint.nodes, 'machine_id', true).then((nodesObj) =>
            setAllNodes(nodesObj)
        );
    ...
    const reducerFromScannedServerToServicesAmount = (accumulated, scannedServer) =>
