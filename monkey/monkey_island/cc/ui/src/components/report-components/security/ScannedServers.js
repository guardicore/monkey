...
import XDataGrid, {
  X_DATA_GRID_CLASSES,
  XDataGridTitle,
} from "../../ui-components/XDataGrid";
import { nanoid } from "nanoid";

const customToolbar = () => {
  return (
    <XDataGridTitle title={"Scanned Servers"} showDataActionsToolbar={false} />
  );
};
...
function getMachineServices(machine) {
  return (
    ...
    >
      {Object.entries(machine?.network_services || {})?.map(
        ([socketAddress, serviceName]) => {
          return (
            <div key={socketAddress}>
              {socketAddress} - {serviceName}
            </div>
          );
        },
...
function ScannedServersComponent(props) {
  ...
  useEffect(() => {
    getCollectionObject(APIEndpoint.nodes, "machine_id", true).then(
      (nodesObj) => setAllNodes(nodesObj),
    );
    getCollectionObject(APIEndpoint.machines, "id", true).then((machinesObj) =>
      setAllMachines(machinesObj),
    );
  }, []);
  ...
  function getScannedMachines() {
    ...
    for (const node of Object.values(allNodes)) {
      for (const [targetMachineId, communications] of Object.entries(
        node.connections,
      )) {
  ...
  const reducerFromScannedServerToServicesAmount = (
    accumulated,
    scannedServer,
  ) => accumulated + scannedServer["services"].length;
  const scannedServicesAmount = props.data.reduce(
    reducerFromScannedServerToServicesAmount,
    0,
  );
