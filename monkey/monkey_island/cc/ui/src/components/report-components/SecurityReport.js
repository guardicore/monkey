...
import {
  getAllTunnels,
  tunnelIssueReportByMachine,
} from "./security/issues/TunnelIssue";
...
class ReportPageComponent extends AuthComponent {
      ...
      return (
        <>
          ...
          <span className="badge text-bg-success">
            {this.state.report.overview.monkey_duration}
          </span>
          , all Agents finished propagation attempts.
        </>
      );
    } else {
      return <></>;
  ...
  generateReportOverviewSection() {
    let manualMonkeyHostnames = getManuallyStartedAgents(this.state.agents).map(
      (agent) =>
        getMachineHostname(getMachineByAgent(agent, this.state.machines)),
    );

    return (
      <div id="overview">
        <h2>Overview</h2>
        <SecurityIssuesGlance
          issuesFound={this.state.report.glance.exploited_cnt > 0}
        />
        {this.state.report.glance.exploited_cnt > 0 ? (
          ...
          <p className="alert alert-info">
            ...
            />
            To improve Infection Monkey's detection rates, try adding
            credentials under <b>Propagation - Credentials</b> and updating
            network settings under <b>Propagation - Network analysis</b>.
          </p>
        )}
        <p>
          The first Infection Monkey Agent ran on{" "}
          <span className="badge text-bg-success">
            {parseTimeToDateString(
              this.state.report.overview.monkey_start_time,
            )}
          </span>
          . {this.getMonkeyDuration()}
        </p>
        <p>
          Infection Monkey started propagating from the following machines where
        ...
        <AvailableCredentials
          stolen={this.state.stolenCredentials}
          configured={this.state.configuredCredentials}
        />
        {this.state.report.overview.config_exploits.length > 0 ? (
          ...
          <p>No exploiters were enabled.</p>
        )}
        ...
        {this.state.report.overview.config_scan ? (
          ...
          <p>
            Note: Infection Monkey was configured to avoid scanning the local
            network.
          </p>
        )}
      </div>
    ...
    return (
      <div id="recommendations">
        ...
        {Object.keys(this.state.report.recommendations.issues).length !== 0 ? (
          <h2>Machine-related Recommendations</h2>
        ) : null}
        <div>
      ...
      return (
        <div>
          ...
          <div>
            ...
            <ul>
              {this.state.report.cross_segment_issues.map((x) =>
                crossSegmentIssueReport(x),
              )}
            </ul>
          </div>
        </div>
  ...
  generateReportGlanceSection() {
    let exploitPercentage =
      (100 * this.state.report.glance.exploited_cnt) /
      this.state.report.glance.scanned.length;
    ...
    return (
      <div id="glance">
        <h2>The Network from Infection Monkey's Eyes</h2>
        <div>
          <p>
            ...
            machines and successfully breached{" "}
        ...
        <div style={{ marginBottom: "20px" }}>
          <p>Infection Monkey stole the following credentials:</p>
  ...
  generateReportFooter() {
    return (
      <div id="footer" className="text-center" style={{ marginTop: "20px" }}>
        ...
        <a href="mailto:support@infectionmonkey.com" className="no-print">
  ...
  generateIssue = (issue) => {
    let remediation = (
      <ReactMarkdown> 'No remediation available'</ReactMarkdown>
    );
    let description = "";

    if (
      _.has(issue, "remediation_suggestion") &&
      issue.remediation_suggestion !== undefined
    ) {
      remediation = (
        <ReactMarkdown
    ...
    let reportContentComponent = (
      <div>
        {remediation}
        <CollapsibleWellComponent>{description}</CollapsibleWellComponent>
      </div>
    );
