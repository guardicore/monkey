...
const getAttackDescription = () => {
  return (
    <>
      After the attacker or malware has propagated through your network, your
      data is at risk on any machine the attacker can access. It can be
      encrypted and held for ransom, exfiltrated, or manipulated in whatever way
    ...
    IslandHttpClient.getJSON(APIEndpoint.agents, {}, true)
      ...
      .then(() =>
        IslandHttpClient.getJSON(APIEndpoint.machines, {}, true)
          ...
          .then(() =>
            IslandHttpClient.getJSON(
              APIEndpoint.agentEvents,
              agent_events_url_args,
              true,
            ).then((res) =>
              setTableData(processEvents(res.body, agents, machines)),
            ),
          ),
      );
  }, []);
...
function getBody(tableData) {
  return (
    <>
      <p>
        Infection Monkey has encrypted <strong>{tableData.length} files</strong>{" "}
        on your network.
      </p>
