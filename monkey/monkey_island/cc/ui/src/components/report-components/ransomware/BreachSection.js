...
const getBreachDescription = () => {
  return (
    <>
      ...
      compromised. The initial compromise was simulated by running Infection
      Monkey Agents manually. Detecting ransomware at this stage will minimize
...
function BreachSection() {
  ...
  if (machines !== null && agents !== null) {
    let manuallyExploitedMachines = getManuallyExploitedMachines(
      agents,
      machines,
    );
    ...
    return (
      <NumberedReportSection
        index={1}
        title={"Breach"}
        description={getBreachDescription()}
        body={body}
      />
    );
  } else {
...
function getManuallyExploitedMachines(agents, machines) {
  ...
  for (let agent of manuallyStartedAgents) {
    ...
    if (machine !== null) {
      ...
      manuallyExploitatedMachine["start_time"] = parseTimeToDateString(
        agent["start_time"],
      );
...
function getMachine(machine) {
  return (
    <li key={machine["hostname"] + machine["start_time"]}>
      ...
      {renderLimitedArray(machine["ip_addresses"], 2, "ip-address")}) at{" "}
      <b>{machine["start_time"]}</b>
    </li>
  );
}
