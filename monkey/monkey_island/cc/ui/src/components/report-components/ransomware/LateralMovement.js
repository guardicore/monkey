...
const getLateralMovementDescription = () => {
    return (
        <>
            After the initial breach, the attacker will begin the Lateral Movement phase of the
            attack. They will employ various techniques in order to compromise other systems in
            your network.
            <br />
            <br />
            <ExternalLink
...
function getScannedVsExploitedStats(num_scanned_nodes, num_exploited_nodes) {
    return (
        <p>
            ...
            <span className="badge text-bg-warning">{num_scanned_nodes}</span> machines and
            successfully breached{' '}
            <span className="badge text-bg-danger">{num_exploited_nodes}</span> of them.
        </p>
    );
}
