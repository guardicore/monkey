...
class MapPageComponent extends AuthComponent {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (
      JSON.stringify(this.props.graph?.nodes) !==
        JSON.stringify(nextProps.graph?.nodes) ||
      JSON.stringify(this.props.graph?.edges) !==
        JSON.stringify(nextProps.graph?.edges)
    ) {
      this.setState({ graphId: nanoid() });
      return true;
    } else if (
      this.state.selected?.machineId !== nextState.selected?.machineId
    ) {
      return true;
    }
    return false;
  ...
  killAllMonkeys = () => {
    this.authFetch(
      ...
      this.setState({ killPressed: true });
    });
  };
  ...
  renderKillDialogModal = () => {
    return (
      <Modal
        show={this.state.showKillDialog}
        onHide={() => this.setState({ showKillDialog: false })}
      >
        <Modal.Body>
          <h2>
