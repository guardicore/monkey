...
function ReportPage() {
  ...
  function selectReport(reports) {
    ...
    for (let report_name in reports) {
      if (
        Object.prototype.hasOwnProperty.call(reports, report_name) &&
        url.endsWith(reports[report_name])
      ) {
  ...
  function getReportFromServer() {
    doesAnyAgentExist(true).then((anyAgentExists) => {
      if (anyAgentExists) {
        authComponent
          .authFetch("/api/report/security", {}, true)
          .then((res) => res.json())
          .then((res) => {
            setSecurityReport(res);
          });
        authComponent
          .authFetch("/api/report/ransomware", {}, true)
          .then((res) => res.json())
          .then((res) => {
  ...
  function renderNavButton(section) {
    return (
      <Nav.Item key={section.key}>
        <Nav.Link key={section.key} eventKey={section.key} onSelect={() => {}}>
          {section.title}
        </Nav.Link>
      </Nav.Item>
    );
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: "1.2em" }}>{renderContent()}</div>
    </Col>
  );
}
