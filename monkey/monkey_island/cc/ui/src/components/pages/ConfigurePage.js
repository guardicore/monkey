...
import UnsafeConfigOptionsConfirmationModal from "../configuration-components/UnsafeConfigOptionsConfirmationModal.js";
import isUnsafeOptionSelected from "../utils/SafeOptionValidator.js";
import {
  getStringsFromBytes,
  getMasqueradesBytesArrays,
  getMasqueradeBytesSubsets,
  MASQUE_TYPES,
...
const CONFIGURED_PROPAGATION_CREDENTIALS_URL =
...
class ConfigurePageComponent extends AuthComponent {
  constructor(props) {
    ...
    this.validator = customizeValidator({
  ...
  componentDidMount = () => {
    this.authFetch(SCHEMA_URL, {}, true)
      .then((res) => res.json())
      .then((schema) => {
    ...
    this.authFetch(CONFIG_URL, {}, true)
      ...
      .then((monkeyConfig) => {
        ...
        this.setState({
          ...
          selectedPlugins: {
            propagation: new Set(
              Object.keys(_.get(monkeyConfig, EXPLOITERS_CONFIG_PATH)),
            ),
            credentials_collectors: new Set(
              Object.keys(
                _.get(monkeyConfig, CREDENTIALS_COLLECTORS_CONFIG_PATH),
              ),
            ),
          },
          sections: sections,
          currentFormData: _.cloneDeep(
            monkeyConfig[this.state.selectedSection],
          ),
        });
      });
  ...
  updateCredentials = () => {
    this.authFetch(CONFIGURED_PROPAGATION_CREDENTIALS_URL, {}, true)
      ...
      .then((credentialsData) => {
        const formattedCredentialsData =
          formatCredentialsForForm(credentialsData);
  ...
  updateMasqueStrings = async () => {
    ...
    const linuxMasqueTexts = getStringsFromBytes(
      linuxMasqueBytes,
      MASQUE_TYPES.TEXTS,
      linuxMasquesSubsets[MASQUE_TYPES.TEXTS.key],
    );
    const linuxMasqueBase64 = getStringsFromBytes(
      linuxMasqueBytes,
      MASQUE_TYPES.BASE64,
      linuxMasquesSubsets[MASQUE_TYPES.BASE64.key],
    );
    ...
    const windowsMasqueTexts = getStringsFromBytes(
      windowsMasqueBytes,
      MASQUE_TYPES.TEXTS,
      windowsMasquesSubsets[MASQUE_TYPES.TEXTS.key],
    );
    const windowsMasqueBase64 = getStringsFromBytes(
      windowsMasqueBytes,
      MASQUE_TYPES.BASE64,
      windowsMasquesSubsets[MASQUE_TYPES.BASE64.key],
    );
    ...
    this.authFetch(CONFIG_URL, {}, true)
      ...
      .then((data) => {
        ...
        this.setState({
          selectedPlugins: {
            propagation: new Set(
              Object.keys(_.get(data, EXPLOITERS_CONFIG_PATH)),
            ),
            credentials_collectors: new Set(
              Object.keys(_.get(data, CREDENTIALS_COLLECTORS_CONFIG_PATH)),
            ),
          },
          configuration: data,
  ...
  setSelectedPlugins = (plugins, key) => {
    this.setState({
      selectedPlugins: { ...this.state.selectedPlugins, [key]: plugins },
    });
  };
  ...
  filterUnselectedPlugins() {
    let pluginTypes = {
      propagation: EXPLOITERS_CONFIG_PATH,
      credentials_collectors: CREDENTIALS_COLLECTORS_CONFIG_PATH,
    };
  ...
  configSubmit(config) {
    ...
    const { linuxMasqueBytes, windowsMasqueBytes } = getMasqueradesBytesArrays(
      this.state.masqueStrings,
    );
    ...
    Promise.all([
      sendCredentialsPromise,
      sendLinuxMasqueStringsPromise,
      sendWindowsMasqueStringsPromise,
    ])
      .then((responses) => {
  ...
  renderConfigExportModal = () => {
    return (
      <ConfigExportModal
        show={this.state.showConfigExportModal}
  ...
  renderConfigImportModal = () => {
    return (
      <ConfigImportModal
        show={this.state.showConfigImportModal}
  ...
  sendCredentials() {
    return this.authFetch(
      CONFIGURED_PROPAGATION_CREDENTIALS_URL,
      {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(
          formatCredentialsForIsland(this.state.credentials.credentialsData),
        ),
      },
      true,
    )
  ...
  renderConfigContent = (displayedSchema) => {
    ...
    if (this.state.selectedSection === "propagation") {
      delete Object.assign(formProperties, {
        configuration: formProperties.formData,
      }).formData;
      return (
        <PropagationConfig
          {...formProperties}
      ...
      return (
        <FormConfig
          ...
          fullUiSchema={fullUiSchema}
          selectedPlugins={
            this.state.selectedPlugins[this.state.selectedSection]
          }
          setSelectedPlugins={this.setSelectedPlugins}
          selectedSection={this.state.selectedSection}
  ...
  renderNav = () => {
    return (
      <Nav
        ...
        onSelect={this.setSelectedSection}
  ...
  isSubmitDisabled = () => {
    ...
    let errors = this.validator.validateFormData(
      this.state.configuration,
      this.state.schema,
    );
    let credentialErrors = this.state.credentials.errors?.length > 0;
    let masqueradeErrors = this.validator.validateFormData(
      this.state.masqueStrings,
      MASQUERADE,
    );
    return (
      errors.errors.length + masqueradeErrors.errors.length > 0 ||
      credentialErrors
    );
  };
  ...
  render() {
    ...
    if (Object.prototype.hasOwnProperty.call(this.state.schema, "properties")) {
      displayedSchema =
        this.state.schema["properties"][this.state.selectedSection];
        ...
        <div>
          ...
          {this.state.lastAction === "invalid_credentials_configuration" ? (
            <div className="alert alert-danger">
              ...
              />
              An invalid configuration file was imported or submitted. One or
              more of the credentials are invalid.
            </div>
          ) : (
            ""
          )}
