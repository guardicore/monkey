    ...
    <div style={{ marginBottom: "2em" }}>
      <div style={{ marginTop: "1em", marginBottom: "1em" }}>
        <p className="alert alert-info">
          ...
          Not sure what this is? Not seeing your AWS EC2 instances?{" "}
          <a
      ...
      {allIPs.length > 1 ? (
        ...
        >
          {allIPs.map((ip) => (
            <Nav.Item key={ip}>
              <Nav.Link eventKey={ip}>{ip}</Nav.Link>
            </Nav.Item>
          ))}
        </Nav>
      ) : (
      ...
      <div className={"aws-run-button-container"}>
        ...
        >
          ...
          {AWSClicked ? (
            <FontAwesomeIcon
              icon={faSync}
              className={`text-success spinning-icon`}
