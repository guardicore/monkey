...
class RunOnIslandButton extends AuthComponent {
  ...
  runIslandMonkey = () => {
    this.setState(
      { runningOnIslandState: MONKEY_STATES.STARTING },
      this.sendRunMonkeyRequest,
    );
  };
        ...
        if (res["is_running"]) {
          ...
          if (res["error_text"] !== "") {
            this.setState({
              ...
              errorMessage: this.getErrorMessageFromErrorText(
                res["error_text"],
              ),
            });
          }
  ...
  getErrorMessageFromErrorText(errorText) {
    if (
      ...
      errorText.includes("Text file busy")
    ) {
  ...
  getMissingBinariesContent() {
    return (
      <span>
        ...
        <a
          href="https://github.com/guardicore/monkey/releases/latest"
          target="blank"
        >
          here
        </a>
  ...
  getMonkeyAlreadyRunningContent() {
    return (
      <span>
        Most likely, monkey is already running on the Island. Wait until it
        finishes or kill the process to run again.
      </span>
    );
  }
  ...
  getUndefinedErrorContent() {
    return (
      <span>
        You encountered an undefined error. Please report it to
        support@infectionmonkey.com or our slack channel.
      </span>
    );
  }
