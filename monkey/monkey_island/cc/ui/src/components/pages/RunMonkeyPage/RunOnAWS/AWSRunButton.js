...
function AWSRunButton(props) {
  ...
  const [awsMachineCollectionError, setAwsMachineCollectionError] =
    useState("");
  ...
  function checkIsOnAWS() {
    authComponent
      .authFetch("/api/remote-monkey?action=list_aws", {}, true)
      .then((res) => res.json())
      .then((res) => {
  ...
  function getErrorDisplay() {
    return (
      <Alert variant={"info"}>
        Detected ability to run on different AWS instances. To enable this
        feature, follow the &nbsp;
        <Button
          variant={"link"}
          className={"inline-link"}
          href={
            "https://techdocs.akamai.com/infection-monkey/docs/running-the-monkey-on-aws-ec2-instances/"
          }
        >
          Tutorial
        </Button>{" "}
        and refresh the page. Error received while trying to list AWS instances:{" "}
        {awsMachineCollectionError}
      </Alert>
    );
  }
