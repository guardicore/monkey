...
const getContents = (props) => {
    ...
    function setUsername(inputVal) {
        if (inputVal) {
            // checks that it's not just whitespaces
    ...
    function generateCommands() {
        if (osType === OS_TYPES.WINDOWS_64) {
            return [
                {
                    type: 'PowerShell',
                    command: GenerateLocalWindowsPowershell(selectedIp, customUsername, otp)
                }
            ];
        } else {
            return [
                {
                    type: 'cURL',
                    command: GenerateLocalLinuxCurl(selectedIp, customUsername, otp)
                },
                {
                    type: 'Wget',
                    command: GenerateLocalLinuxWget(selectedIp, customUsername, otp)
                }
            ];
        }
    }
    ...
    return (
        <>
            ...
            <DropdownSelect
                defaultKey={OS_TYPES.WINDOWS_64}
                options={osTypes}
                onClick={setOsType}
            ...
            <p style={{ fontSize: '1.2em' }}>Run as a user by entering their username:</p>
            <div>
                <Form>
                    <Form.Control
                        type="text"
            ...
            <div style={{ textAlign: 'right' }}>
                <span>
                    <Button title="Copy to Clipboard" onClick={getOtp}>
                        Refresh OTP
                    </Button>
                </span>
            </div>
