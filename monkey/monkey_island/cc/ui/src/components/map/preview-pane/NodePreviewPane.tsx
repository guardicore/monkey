...
const NodePreviewPane = (props: any) => {
  ...
  function ipsRow(node) {
    return (
      <tr>
        <th>IP Addresses</th>
        <td>
          {node.networkInterfaces.map((val) => (
            <div key={val}>{val}</div>
          ))}
        </td>
      </tr>
  ...
  function downloadLogsRow(node) {
    const agentLogDownloadDisabled =
      _.isEmpty(node.agentIds) || node.agentRunning;
    return (
      <>
        <tr>
          <th>Download Monkey Agent Log</th>
          <td>
            <AgentLogDownloadButton
              url={"/api/agent-logs/"}
        ...
        {node.island && (
          <tr>
            <th>Download Island Server Log</th>
            <td>
