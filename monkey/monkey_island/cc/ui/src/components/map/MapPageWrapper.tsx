...
const MapPageWrapper = (props) => {
    function getPropagationEvents(refreshToken: boolean) {
        ...
        return IslandHttpClient.getJSON(APIEndpoint.agentEvents, url_args, refreshToken).then(
            (res) => arrayToObject(res.body, 'target')
        );
    }
    ...
    const [graphSnapshot, setGraphSnapshot] = useState<Graph>({
        edges: [],
        nodes: []
    });

    async function fetchMapNodes(refreshToken: boolean) {
        await getCollectionObject(APIEndpoint.nodes, 'machine_id', refreshToken).then((nodeObj) =>
            setNodes(nodeObj)
        );
        await getCollectionObject(APIEndpoint.machines, 'id', refreshToken).then((machineObj) =>
            setMachines(machineObj)
        );
