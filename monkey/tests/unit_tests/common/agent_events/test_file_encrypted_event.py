...
LINUX_FILE_ENCRYPTED_EVENT = FileEncryptedEvent(
    source=AGENT_ID,
    timestamp=TIMESTAMP,
...
WINDOWS_FILE_ENCRYPTED_EVENT = FileEncryptedEvent(
    source=AGENT_ID,
    timestamp=TIMESTAMP,
    target=IPv4Address(TARGET_IP_STR),
...
LINUX_FILE_ENCRYPTED_OBJECT_DICT = {
    "source": AGENT_ID,
    "timestamp": TIMESTAMP,
    "target": IPv4Address(TARGET_IP_STR),
    "file_path": LINUX_FILE_ENCRYPTED_PATH,
...
LINUX_FILE_ENCRYPTED_SIMPLE_DICT = {
    "source": str(AGENT_ID),
    "timestamp": TIMESTAMP,
...
@pytest.mark.parametrize(
    "linux_file_encrypted_event_dict",
    [LINUX_FILE_ENCRYPTED_OBJECT_DICT, LINUX_FILE_ENCRYPTED_SIMPLE_DICT],
)
def test_constructor(linux_file_encrypted_event_dict):
    assert FileEncryptedEvent(**linux_file_encrypted_event_dict) == LINUX_FILE_ENCRYPTED_EVENT


def test_serialization():
    serialized_event = LINUX_FILE_ENCRYPTED_EVENT.dict(simplify=True)
    assert serialized_event == LINUX_FILE_ENCRYPTED_SIMPLE_DICT
...
def test_construct_invalid_field__type_error(key, value):
    invalid_type_dict = LINUX_FILE_ENCRYPTED_SIMPLE_DICT.copy()
...
def test_construct_invalid_field__value_error(key, value):
    invalid_value_dict = LINUX_FILE_ENCRYPTED_SIMPLE_DICT.copy()
    invalid_value_dict[key] = value

    with pytest.raises(ValueError):
        FileEncryptedEvent(**invalid_value_dict)


def test_construct__extra_fields_forbidden():
    extra_field_dict = LINUX_FILE_ENCRYPTED_SIMPLE_DICT.copy()
...
@pytest.mark.parametrize(
    "event",
    [LINUX_FILE_ENCRYPTED_EVENT, WINDOWS_FILE_ENCRYPTED_EVENT],
)
def test_file_encrypted_event__de_serialization(event):
    serialized_event = event.dict(simplify=True)
    deserialized_event = FileEncryptedEvent(**serialized_event)
