...
def _cast_by_example(value, example):
    ...
    if example_type is str:
        ...
        return float(value)
    elif example_type is type or example_type is ABCMeta:
...
class Configuration(object):
    ...
    def as_dict(self):
        ...
        for key in dir(Configuration):
            ...
            if val_type is type or val_type is ABCMeta:
                value = value.__name__
            elif val_type is tuple or val_type is list:
                if len(value) != 0 and (type(value[0]) is type or type(value[0]) is ABCMeta):
